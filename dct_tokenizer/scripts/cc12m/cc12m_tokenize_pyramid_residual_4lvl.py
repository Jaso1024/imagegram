#!/usr/bin/env python3
"""Tokenize CC12M WebDataset shards into fixed-length 4-level pyramid+residual tokens.
...
Levels:
  ...
  L3: residual 64x64 RGB vs upsample(recon32->64) (12288 tokens, stored as residual+128 clipped to [0,255])

Total tokens/image = 192 + 768 + 3072 + 12288 = 16320, plus separator 65535.
...
class PyramidResidualTokenizer4:
    ...
    def tokenize_pil(self, img: Image.Image) -> np.ndarray:
        ...
        recon32 = np.clip(up32 + (resid32_q.astype(np.int16) - 128), 0, 255).astype(np.uint8)

        # up recon32 -> 64
        up64 = np.asarray(Image.fromarray(recon32, mode="RGB").resize((self.l3, self.l3), Image.BILINEAR), dtype=np.int16)
        resid64 = a64 - up64
        resid64_q = np.clip(resid64 + 128, 0, 255).astype(np.uint8)

        t = np.empty((self.tokens_per_image,), dtype=np.uint16)
        o0 = self.base * self.base * 3
        o1 = o0 + self.l1 * self.l1 * 3
        o2 = o1 + self.l2 * self.l2 * 3

        t[:o0] = a8.astype(np.uint8).reshape(-1).astype(np.uint16)
        t[o0:o1] = resid16_q.reshape(-1).astype(np.uint16)
        t[o1:o2] = resid32_q.reshape(-1).astype(np.uint16)
        t[o2:] = resid64_q.reshape(-1).astype(np.uint16)
        return t


def tokenize_one_tar(tar_path: str, out_bin: str) -> Tuple[str, int, int]:
    tok = PyramidResidualTokenizer4(8)
    images = 0
    failed = 0
...
def main():
    ...
    ap.add_argument("--out-dir", default="/root/cc12m_tokens_4lvl")
    ...
    tok = PyramidResidualTokenizer4(8)
    cfg = {
        "tokenizer": "pyramid_residual_4lvl",
        "encoded_levels": [8, 16, 32, 64],
