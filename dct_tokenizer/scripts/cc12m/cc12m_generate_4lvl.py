#!/usr/bin/env python3
"""Generate images by n-gram sampling from CC12M 4-level pyramid residual tokens.

Token format (per image):
  base 8x8 RGB: 192 tokens [0..255]
  resid16: 768 tokens [0..255] (residual+128)
  resid32: 3072 tokens [0..255] (residual+128)
  resid64: 12288 tokens [0..255] (residual+128)
Total: 16320 tokens.

Decode:
  base8 -> up16 + (resid16-128) -> recon16
  recon16 -> up32 + (resid32-128) -> recon32
  recon32 -> up64 + (resid64-128) -> recon64
  recon64 -> upsample to output_size.

Uses MultiShardGramEngine to fan-out across shard indices.

Warning: generation is slower because sequences are much longer.
"""
...
def decode_4lvl(tokens: np.ndarray, output_size: int = 256) -> np.ndarray:
    assert tokens.shape[0] == 16320

    off0 = 192
    off1 = off0 + 768
    off2 = off1 + 3072

    base = tokens[:off0].astype(np.uint8).reshape(8, 8, 3)
    resid16_q = tokens[off0:off1].astype(np.int16).reshape(16, 16, 3)
    resid32_q = tokens[off1:off2].astype(np.int16).reshape(32, 32, 3)
    resid64_q = tokens[off2:].astype(np.int16).reshape(64, 64, 3)
    ...
    recon64 = np.clip(up64 + (resid64_q - 128), 0, 255).astype(np.uint8)

    if output_size != 64:
        out = Image.fromarray(recon64, mode="RGB").resize((output_size, output_size), Image.BILINEAR)
...
def main():
    ...
    ap.add_argument("--out-dir", default="/root/cc12m_generated_4lvl")
    ...
    assert tpi == 16320, f"Expected 16320 tokens/image, got {tpi}"
    ...
    for i in range(args.num_images):
        ...
        img = decode_4lvl(toks, output_size=args.output_size)
