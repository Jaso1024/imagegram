#!/usr/bin/env python3
"""Generate images by n-gram sampling from CC12M 5-level pyramid residual tokens.

Levels: 8,16,32,64,128 (then upsample to output_size)
Tokens per image: 65472.

Decode:
  base8 -> up16 + (r16-128) -> recon16
  recon16 -> up32 + (r32-128) -> recon32
  recon32 -> up64 + (r64-128) -> recon64
  recon64 -> up128 + (r128-128) -> recon128
  recon128 -> upsample to output_size.

Uses MultiShardGramEngine fan-out across shard indices.

WARNING: generation is slow because sequences are 65k tokens long.
"""
...
def decode_5lvl(tokens: np.ndarray, output_size: int = 256) -> np.ndarray:
    assert tokens.shape[0] == 65472
    ...
    base8 = tokens[offs[0] : offs[1]].astype(np.uint8).reshape(8, 8, 3)
    r16 = tokens[offs[1] : offs[2]].astype(np.int16).reshape(16, 16, 3)
    r32 = tokens[offs[2] : offs[3]].astype(np.int16).reshape(32, 32, 3)
    r64 = tokens[offs[3] : offs[4]].astype(np.int16).reshape(64, 64, 3)
    r128 = tokens[offs[4] : offs[5]].astype(np.int16).reshape(128, 128, 3)
...
def main():
    ...
    ap.add_argument("--index-root", default="/root/cc12m_indices_5lvl")
    ap.add_argument("--config", default="/root/cc12m_tokens_5lvl/config.json")
    ap.add_argument("--out-dir", default="/root/cc12m_generated_5lvl")
    ...
    for i in range(args.num_images):
        ...
        img = decode_5lvl(toks, output_size=args.output_size)
